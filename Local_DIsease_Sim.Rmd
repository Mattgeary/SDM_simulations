...
* Mean temperature between 20.0 and 26.5 degrees C
* Precipitation above 50 and below 180

```{r DiseaseRules, echo=FALSE, include=FALSE}
rcl <- matrix(c(0, 209.999,0, 209.9999, 265, 1, 256.0001, 315,0), nrow=3, ncol=3, byrow=T)
...
rcl <- matrix(c(0, 49.999, 0,49.9999, 180.0001, 1, 180.0002, 365, 0), nrow=3, ncol=3, byrow=T)
...
comb.suit <- crop(comb.suit, extent(c(29.32717, 41.88752, -11.74569,  4.633819)))
```
...
hetero.pres.comb <- hetero.pres.comb[sample(c(1:length(hetero.pres.comb[,1])), 100),]
rm(ab)
...
for(i in 1:runs){
  ...
  hetero.pts.africa <- randomPoints(comb.suit, 1000)
  ...
  hetero.pres.africa <- cbind(hetero.pts.africa, pa = extract(comb.suit, hetero.pts.africa))
